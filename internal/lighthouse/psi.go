...
func FetchPSI(ctx context.Context, pageURL, apiKey, strategy string) (*PSIResult, error) {
	endpoint := psiEndpoint(pageURL, apiKey, strategy)
...
func psiEndpoint(pageURL, apiKey, strategy string) string {
	return fmt.Sprintf(
		...
		url.QueryEscape(pageURL), url.QueryEscape(apiKey), url.QueryEscape(strategy),
	)
}
